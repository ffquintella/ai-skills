---
name: agent-router
description: Decides which agent, Claude model and effort level should handle a task before any work starts, so the coordinator spends the fewest tokens that still hold quality. Use this every time the user asks to implement, fix, refactor, test, review, investigate, document or plan anything, even when the task looks small (the router may answer "inline" for trivial work). Also use it when unsure whether to delegate at all or how many agents to spawn.
argument-hint: [one-line task description]
---

# Agent router (Claude Code edition)
...
summary. The two cost levers are the **model tier** and the **effort level**
(`low` < `medium` < `high` < `xhigh` < `max`).

| Tier | Model id | Input $/1M | Output $/1M | Relative input cost |
| --- | --- | --- | --- | --- |
| Haiku 4.5 | `claude-haiku-4-5` | 1.00 | 5.00 | 1x |
| Sonnet 5 | `claude-sonnet-5` | 2.00 | 10.00 | 2x |
| Opus 5 | `claude-opus-5` | 5.00 | 25.00 | 5x |
| Fable 5.1 | `claude-fable-5-1` | 10.00 | 50.00 | 10x |

Anthropic's measurements say: sweep effort before changing model, lower effort on a
newer model often beats higher effort on an older one, and "run cheap, re-run failures
higher" gives the same pass rate at about half the cost. Details and URLs in
[references/sources.md](references/sources.md).

Effort can only be pinned in an agent definition, never per call, so the project ships
one agent per tier in `.claude/agents/`. Pick from those. If the project does not have
them yet, copy the templates in [assets/agents/](assets/agents/) and fill in the
...
| **review / security** | Judge an existing diff or design against the project rules (AGENTS.md / CLAUDE.md). No edits. |
...
| review / security | `reviewer` | sonnet (override `model: opus` for security-sensitive paths: auth, secrets, rate limiting) | high | 1 |
...
2. **Boundaries**: files or directories in scope, what not to touch, project rules that apply (from AGENTS.md / CLAUDE.md: auth conventions, endpoint catalogues, routing, no secrets in repo).
...
- Do not read large files or run the full test suite without a filter in the main context when an agent can return the summary instead.
