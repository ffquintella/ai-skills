...
[cdeust/Cortex](https://github.com/cdeust/Cortex)). On Claude Code it ships as a plugin
whose lifecycle hooks make memory *ambient*: context is injected at session start, every
prompt triggers an auto-recall, significant tool output is captured, compaction writes a
checkpoint. On every other host the same tools exist but nothing happens unless called.
...
Tools are namespaced `mcp__plugin_hypermnesia-mcp_cortex__<tool>`; this document writes
them bare (`recall`, `consolidate`, …). The full catalogue is in
...
| --- | --- |
| No `mcp__plugin_hypermnesia-mcp_cortex__*` tools in the session | Plugin not installed, or the session predates the install — restart it |
